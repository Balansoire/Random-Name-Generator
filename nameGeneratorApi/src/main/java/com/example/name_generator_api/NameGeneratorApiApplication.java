...
public class NameGeneratorApiApplication {
	...
	public static void main(String[] args) {
		...
		NameGeneratorService generator = new NameGeneratorService();
		...
		while(running) {
			try {
				...
				String word ="Erreur";
				if (key == 32) { // ESPACE
					word = generator.generateName("naive", null, list);
				} else if (48 <= key && key <= 57) { // 0-9
					String[][] arg = {{"sectionSize", String.valueOf(key - 48)}};
					word = generator.generateName("sectional", arg, list);
				} else if (key == 109) { // m
					word = generator.generateName("multiSectional", null, list);
				} else if (key == 9) { // TAB
					running = false;
