...
import com.example.name_generator_api.strategy.MultiSectionalStrategy;
...
public class NameGeneratorService {
	...
	public String generateName(String strategyName, String[][] strategyArgs, NameList list) throws EmptyListException, NotImplementedException, BadArgsException, ListOfEmptyException {
		if (instances == null) instances = new ArrayList<NameGenerationStrategy>();
		...
		switch(strategyName) {
			...
			case "multiSectional":
				for (NameGenerationStrategy strategy : instances) {
					if (strategy.name.equals("multiSectional")) {
						exists = true;
						return strategy.generateName(list);
					}
				}
				if (!exists) {
					NameGenerationStrategy instance = new MultiSectionalStrategy();
					instances.add(instance);
					return instance.generateName(list);
				}
			default:
